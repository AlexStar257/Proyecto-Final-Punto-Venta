...
controller.save = (req, res) => {
  ...
  pool.query('INSERT INTO productos SET ?', [data], (err, productos) => {
    if (err) {
      console.log(err);
...
controller.update = (req, res) => {
  ...
  pool.query('UPDATE productos set ? WHERE id = ?', [nuevoProducto, id], (err, rows) => {
    if (err) {
      console.log(err);
    }
    res.redirect('/productos');
...
controller.delete = (req, res) => {
  const { id } = req.params;
  pool.query('DELETE FROM productos WHERE id = ?', [id], (err, rows) => {
    res.redirect('/productos');
  })
}
...
controller.estado = (req, res) => {
  const { id } = req.params;
  pool.query('SELECT estado FROM productos WHERE id = ?', [id], (err, rows) => {
    if (err) {
      console.log(err);
      res.status(500).json({ message: 'Error interno del servidor' });
      return;
    ...
    pool.query('UPDATE productos SET estado = ? WHERE id = ?', [nuevoEstado, id], (err, rows) => {
      if (err) {
        console.log(err);
        res.status(500).json({ message: 'Error interno del servidor' });
        return;
